...
export default {
	...
	data() {
		return {
			myIncomeLogs: [],
			myIncomeLogsToday: [],
			myIncomeLogsThisMonth: [],
			logToDisplay: this.myIncomeLogsToday,
			myIncomeSources: [],
	...
	mounted() {
		this.getMyIncomeLogs();
		this.getMyIncomeSources();
	},
	methods: {
		...
		filterByInterval(event) {
			...
			// console.log(this.myIncomeLogs);
			// console.log(this.myIncomeLogsThisMonth);
			// console.log(this.myIncomeLogsToday);

			setTimeout(() => {
				if (event.target.value === "All") {
					this.logFilter = event.target.value;
					this.logToDisplay = this.myIncomeLogs;
					return;
				} else if (event.target.value === "Today") {
					this.logFilter = event.target.value;
					this.logToDisplay = this.myIncomeLogsToday;
					return;
				} else if (event.target.value === "This Month") {
					this.logFilter = event.target.value;
					this.logToDisplay = this.myIncomeLogsThisMonth;
					return;
				}
			}, 100);
		},
		addNewLog() {
			if (this.newLogAmount < 0) {
				// display error
				return;
			}
			...
			if (this.newLogSource !== "") {
				...
				let newSourceId = -99;
				let newBudgetId = -99;
				for (const index in this.myIncomeSources) {
					if (
						this.myIncomeSources[index].sourceName ===
						this.newLogSource
					) {
						newSourceId = this.myIncomeSources[index].sourceId;
					}
				}
		...
		getMyIncomeLogs() {
			...
			Vue.axios
				...
				.then((res) => {
					...
					this.myIncomeLogs = retrievedLogs;
					// console.log(this.myIncomeLogs);
					...
					let filteredLog = [];
					for (const index in this.myIncomeLogs) {
						const currentLogDate =
							this.myIncomeLogs[index].logDate.split("-");
						if (
							currentLogDate[0] === thisYear &&
							currentLogDate[1] === thisMonth &&
							currentLogDate[2] === thisDate
						) {
							filteredLog.push(this.myIncomeLogs[index]);
						}
					}
					this.myIncomeLogsToday = filteredLog;
					...
					this.myIncomeLogsThisMonth = filteredLog;
		...
		getMyIncomeSources() {
			...
			Vue.axios
				...
				.then((res) => {
					console.log(res);
					for (const arrIndex in res.data) {
						if (res.data[arrIndex].sourceType == "INCOME") {
							let sourceObj = {
					...
					this.myIncomeSources = retrievedSources;
					console.log(this.myIncomeSources);
				});
		},
